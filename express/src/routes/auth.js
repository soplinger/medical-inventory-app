...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const logins = JSON.stringify({ ip: userIp, timestamp: new Date().toISOString() });

    const perms = 'login';

    // Insert the new user into the database
    await pool.query('INSERT INTO users (email, first, last, logins, password, perms, role, salt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [email, first, last, logins, hashedPassword, perms, role, salt]);
